...
from sp_api.base import SellingApiException, SellingApiServerException
...
def remove_b2b_price(sku, product_type):
    current_listing = get_listing_details(
        sku=sku,
        include=['attributes']
    )
    purchasable_offers = current_listing.payload.get('attributes', {}).get('purchasable_offer', [])

    b2b_offers_only = [
        offer for offer in purchasable_offers
...
def close_listing(sku, product_type):
    ...
    fulfillment_availability = current_listing.payload.get('attributes', {}).get('fulfillment_availability', [])
...
def batch_delete_listings(skus):
    confirmation = input(f'You are about to delete {len(skus)} listings, please confirm by typing "Yes": ')
    if confirmation != "Yes":
        print("Exiting...")
        return
    error_skus = {}
...
def batch_close_listings(skus):
    ...
    for i, sku in enumerate(skus, start=1):
        ...
        response = get_listing_details(sku=sku, include=['productTypes','fulfillmentAvailability'])
        time.sleep(1/5)
        if not response.error:
            ...
            if (
                    ...
                    and payload['fulfillmentAvailability'][0]['fulfillmentChannelCode']=='AMAZON_NA'
                ):
...
result = batch_delete_listings(skus)
